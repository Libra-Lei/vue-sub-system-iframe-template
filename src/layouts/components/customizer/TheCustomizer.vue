        ...
        <VuePerfectScrollbar class="scroll-area--customizer pt-4 pb-6" :settings="settings">
          ...
          <div class="px-6">
            ...
            <!-- <div class="mt-4">
              ...
              <div>
                <vs-radio v-model="layoutType" vs-value="vertical" class="mr-4" vs-name="layout-type-vertical">垂直导航</vs-radio>
                <vs-radio v-model="layoutType" vs-value="horizontal" class="mr-4" vs-name="layout-type-horizontal">水平导航</vs-radio>
              </div>
            </div>
            ...
            <div class="mt-4">
              ...
              <div>
                <vs-radio v-model="themeMode" vs-value="light" class="mr-4" vs-name="theme-mode-light">浅色</vs-radio>
                <vs-radio v-model="themeMode" vs-value="dark" class="mr-4" vs-name="theme-mode-dark">深色</vs-radio>
                <vs-radio
                  v-if="layoutType === 'vertical'"
                  v-model="themeMode"
                  vs-value="semi-dark"
                  vs-name="theme-mode-semi-dark">半暗</vs-radio>
              </div>
            </div>
            ...
            <!-- NAVBAR TYPE -->
            <div class="mt-4">
              <h5 class="mb-2">{{ (layoutType === 'vertical' || windowWidth &lt; 1200) ? "顶栏" : "菜单" }} - 模式</h5>
              <div>
                <vs-radio
                ...
                <vs-radio v-model="navbarTypeLocal" vs-value="sticky" vs-name="navbar-type-sticky" class="mr-4">吸顶</vs-radio>
                <vs-radio v-model="navbarTypeLocal" vs-value="floating" vs-name="navbar-type-floating">悬浮</vs-radio>
              </div>
            </div>
...
export default {
  ...
  computed: {
    ...
    layoutType: {
      get()    { return this.$store.state.app.mainLayoutType },
      set(val) { this.$store.commit("app/UPDATE_MAIN_LAYOUT_TYPE", val) }
    },
    primaryColor: {
      get()    { return this.$store.state.app.themePrimaryColor },
      set(val) { this.$store.commit('app/UPDATE_PRIMARY_COLOR', val) }
    },
    reduced_sidebar: {
      get()    { return this.$store.state.app.reduceButton },
      set(val) { this.$store.commit('app/TOGGLE_REDUCE_BUTTON', val) }
    },
    ...
    themeMode: {
      get()    { return this.$store.state.app.theme },
      set(val) { this.$store.dispatch('app/updateTheme', val) }
    },
    windowWidth() {
