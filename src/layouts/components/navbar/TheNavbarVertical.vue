...
export default {
    ...
    computed: {
        navbarColorLocal() {
          return this.$store.state.app.theme === "dark" ? "#10163a" : this.navbarColor
        },
        // HELPER
        ...
        starredPagesLimited: {
            ...
            set(list) {
                this.$store.dispatch('app/arrangeStarredPagesLimited', list);
            }
        },
        starredPagesMore: {
            get() {
            ...
            set(list) {
                this.$store.dispatch('app/arrangeStarredPagesMore', list);
            }
        },
    ...
    methods: {
        ...
        actionClicked(item) {
            // e.stopPropogation();
            this.$store.dispatch('app/updateStarredPage', { index: item.index, val: !item.highlightAction });
        },
        showNavbarSearch() {
