...
export default{
	...
	watch: {
		...
		searchQuery(val) {
			if(val == '') {
				this.inputInit();
				if(this.bodyOverlay) this.$store.commit('app/TOGGLE_CONTENT_OVERLAY', false);
			}else {
				if(this.backgroundOverlay && !this.bodyOverlay) this.$store.commit('app/TOGGLE_CONTENT_OVERLAY', true);
	...
	methods: {
		...
		suggestionSelected() {
			if(this.bodyOverlay && this.filteredData[0].url) this.$store.commit('app/TOGGLE_CONTENT_OVERLAY', false);
